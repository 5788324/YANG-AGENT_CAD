...
项目刚初始化。GitHub 仓库已克隆到本地工作区，AI-first 开发规范、安全工作流和最小 Python CLI 骨架已建立。
...
1. 增加开发环境安装说明，解决普通 `python/py` 不在 PATH 的问题。
2. 实现备份/回滚基础模块。
3. 实现 LISP validator 基础规则。
4. 实现 accoreconsole runner 原型。
5. 实现 MCP Server 最小工具：doctor、插件列表、任务查询。
